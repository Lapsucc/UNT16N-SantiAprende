...
namespace Coffee.UIExtensions
{
    internal static class UIParticleUpdater
    {
        ...
        private static void ModifyScale(UIParticle particle)
        {
            ...
            Vector3 s = particle.canvas.rootCanvas.transform.localScale;
            Vector3 modifiedScale = new(
        ...
        private static Matrix4x4 GetScaledMatrix(ParticleSystem particle)
        {
            ...
            ParticleSystemSimulationSpace space = main.simulationSpace;
        ...
        private static void BakeMesh(UIParticle particle)
        {
            ...
            Matrix4x4 rootMatrix = Matrix4x4.Rotate(root.rotation).inverse
                             * Matrix4x4.Scale(root.lossyScale).inverse;
            Vector3 scale = particle.ignoreCanvasScaler
            ...
            Vector3 diff = position - particle.cachedPosition;
            ...
            for (int i = 0; i < particle.particles.Count; i++)
            {
                ...
                if (currentPs.main.simulationSpace == ParticleSystemSimulationSpace.World && 0 < diff.sqrMagnitude)
                {
                    ...
                    int count = currentPs.particleCount;
                    ...
                    {
                        ParticleSystem.Particle p = s_Particles[j];
                        p.position += diff;
                        s_Particles[j] = p;
                    }
                ...
                if (CanBakeMesh(r))
                {
                    ...
                    long hash = currentPs.GetMaterialHash(false);
                    if (hash != 0)
                    {
                        Mesh m = MeshHelper.GetTemporaryMesh();
                ...
                if (currentPs.trails.enabled)
                {
                    ...
                    long hash = currentPs.GetMaterialHash(true);
                    if (hash != 0)
                    {
