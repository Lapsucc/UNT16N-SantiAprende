...
namespace Coffee.UIExtensions
{
...
#endif
    {
        ...
        private readonly List<Material> _modifiedMaterials = new();
        private readonly List<Material> _maskMaterials = new();
        private readonly List<bool> _activeMeshIndices = new();
        private Vector3 _cachedPosition;
        private static readonly List<Material> s_TempMaterials = new(2);
        private static MaterialPropertyBlock s_Mpb;
        private static readonly List<Material> s_PrevMaskMaterials = new();
        private static readonly List<Material> s_PrevModifiedMaterials = new();
        ...
        protected override void UpdateMaterial()
        {
            ...
            // No mesh to render.
            int count = activeMeshIndices.CountFast();
            ...
            int materialCount = Mathf.Min(8, count);
            ...
            for (int i = 0; i < particles.Count; i++)
            {
                ...
                {
                    Material mat = GetModifiedMaterial(s_TempMaterials[0], ps.GetTextureForSprite());
                ...
                {
                    Material mat = GetModifiedMaterial(s_TempMaterials[1], null);
                    for (int k = 1; k < s_Components.Count; k++)
                        mat = (s_Components[k] as IMaterialModifier).GetModifiedMaterial(mat);
                    canvasRenderer.SetMaterial(mat, j++);
                }
        ...
        {
            foreach (Material m in s_PrevMaskMaterials)
                StencilMaterial.Remove(m);
            s_PrevMaskMaterials.Clear();

            foreach (Material m in s_PrevModifiedMaterials)
        ...
        private Material GetModifiedMaterial(Material baseMaterial, Texture2D texture)
        {
            ...
            int id = m_AnimatableProperties.Length == 0 ? 0 : GetInstanceID();
        ...
        internal void UpdateMaterialProperties()
        {
            ...
            //
            int count = activeMeshIndices.CountFast();
            int materialCount = Mathf.Max(8, count);
            canvasRenderer.materialCount = materialCount;
            int j = 0;
            for (int i = 0; i < particles.Count; i++)
            {
        ...
        internal void UpdateMaterialProperties(Renderer r, int index)
        {
            ...
            Material mat = canvasRenderer.GetMaterial(index);
            if (!mat) return;
        ...
        private new IEnumerator Start()
        {
            ...
            bool delayToPlay = particles.AnyFast(ps =>
            {
