...
namespace Coffee.UIExtensions
{
    internal class AnimatedPropertiesEditor
    {
        ...
        static string CollectActiveNames(SerializedProperty sp, List<string> result)
        {
            ...
            {
                SerializedProperty spName = sp.GetArrayElementAtIndex(i).FindPropertyRelative("m_Name");
        ...
        public static void DrawAnimatableProperties(SerializedProperty sp, Material[] mats)
        {
            ...
            {
                Rect r = EditorGUI.PrefixLabel(EditorGUILayout.GetControlRect(true), new GUIContent(sp.displayName, sp.tooltip));
                string text = sp.hasMultipleDifferentValues ? "-" : CollectActiveNames(sp, s_ActiveNames);
            ...
            if (!sp.hasMultipleDifferentValues)
            {
                for (int i = 0; i < sp.arraySize; i++)
                {
                    ...
                    string name = p.FindPropertyRelative("m_Name").stringValue;
                    ShaderPropertyType type = (ShaderPropertyType) p.FindPropertyRelative("m_Type").intValue;
            ...
            foreach (Material mat in mats)
            {
                ...
                for (int i = 0; i < ShaderUtil.GetPropertyCount(mat.shader); i++)
                {
                    ...
                    ShaderPropertyType type = (ShaderPropertyType) ShaderUtil.GetPropertyType(mat.shader, i);
                    string name = string.Format("{0} ({1})", pName, type);
        ...
        private static void AddMenu(GenericMenu menu, SerializedProperty sp, AnimatedPropertiesEditor property, bool add)
        {
            ...
            {
                int index = s_ActiveNames.IndexOf(property._name);
                if (0 <= index)
                {
